...
c     Grid description
      ...
      real,allocatable, dimension (:,:,:) :: zz        ! 3d height
      real,allocatable, dimension (:,:)   :: zzs       ! surface height
...
c     Allocate memory for 3d arrays: pressure, theta, pv
      ...
      if (stat.ne.0) print*,'*** error allocating array zz ***'
      allocate(zzs(nx,ny),stat=stat)
      if (stat.ne.0) print*,'*** error allocating array zzs **'
...
c     Vertical mode <m>
      elseif ( umode.eq.'m' ) then

         do i=1,start_n
            call get_index3(rid,rjd,rkd,start_lon(i),start_lat(i),
     >                start_lev(i),1,zz,zzs,nx,ny,nz,xmin,ymin,dx,dy)
            tmp1 = int_index3 (pr,nx,ny,nz,rid,rjd,rkd,mdv)
            start_pre(i) = tmp1
         enddo
...
c           Get height at grid point
            elseif ( umode_save.eq.'m' ) then

               call get_index3(rid,rjd,rkd,start_lon(i),start_lat(i),
     >                         start_pre(i),3,pr,prs,nx,ny,nz,
     >                         xmin,ymin,dx,dy)
               tmp1 = int_index3 (zz,nx,ny,nz,rid,rjd,rkd,mdv)
