...
set flag = `${LAGRANTO}/create_startf.perl "${specifier}" | tail -1` 
if ( "${flag}" != "nil" ) then
    set format1 = ".3"
    set format2 = ".3"
else
...
echo    
echo "PROGRAM CREATE_STARTF : ${LAGRANTO}/create_startf"
...
if ( ( -f ${file} ) && ( ${min} == 0 ) ) then
  ...
  set file=`ls ${charp}????????_??* | head -1 | sed -e 's/@//'`
endif
...
foreach i ( ${charp}????????_?? )
  ...
  set td1  = `${LAGRANTO}/gettidiff ${yyyy}${mm}${dd}_${hh} ${date0}`
...
if ( ( ${timeshift} != 0 ) || ( ${min} > 0 ) ) then
    set date1=`${LAGRANTO}/newtime $date0 $timeinc`
...
if ( "${changet}" == "true" ) then
  ...
  echo   
  ${LAGRANTO}/changet.sh ${refdate} ${pfile0}
  ${LAGRANTO}/changet.sh ${refdate} ${pfile1}
  if ( -f  ${sfile0} ) then
    ${LAGRANTO}/changet.sh ${refdate} ${sfile0}
  endif
  if ( -f  ${sfile1} ) then
    ${LAGRANTO}/changet.sh ${refdate} ${sfile1}
  endif
endif
...
${LAGRANTO}/create_startf.perl "${specifier}" >> ${parfile}
