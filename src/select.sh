...
set times     =  `${LAGRANTO}/trainfo.sh ${inpfile} times`
set vars      =  `${LAGRANTO}/trainfo.sh ${inpfile} vars`

# Split the criterion into subunits (with Perl)
${LAGRANTO}/select.perl "${crit}" >! ${crifile} 
if ( "${status}" != "0" ) then
...
echo "PROGRAM SELECT        : ${LAGRANTO}/select"
echo "PARSER                : ${LAGRANTO}/select.perl"
