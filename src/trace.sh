...
set startdate = `${LAGRANTO}/trainfo.sh ${inpfile} startdate` 
set enddate   = `${LAGRANTO}/trainfo.sh ${inpfile} enddate` 
set refdate   = `${LAGRANTO}/trainfo.sh ${inpfile} refdate` 
set ntra      = `${LAGRANTO}/trainfo.sh ${inpfile} ntra`
...
set timediff_hh = `${LAGRANTO}/gettidiff ${enddate_ymdh} ${startdate_ymdh}`
...
if ( ${timeinc} == "nil" ) then
  ...
  set timeinc=`${LAGRANTO}/gettidiff $date2 $date1`
endif
if ( $timeinc == 0 ) then
...
set td1  = `${LAGRANTO}/gettidiff ${startdate_ymdh} ${date}`
set td2  = `${LAGRANTO}/gettidiff ${enddate_ymdh}   ${date}`
...
if ( $direction == "f" ) then
  set tdiff0 = `${LAGRANTO}/gettidiff ${startdate_ymdh} ${date}`
else
  set tdiff0 = `${LAGRANTO}/gettidiff ${enddate_ymdh} ${date}`
endif
  if ( $tdiff0 < 0 ) then
...
while ( $num > 1 )
  ...
  set date=`${LAGRANTO}/newtime $date $timeinc`
...
if ( $direction == f ) then
  set tstart = `${LAGRANTO}/gettidiff $startdate $datfiles[1]`
  set tend   = `${LAGRANTO}/gettidiff $datfiles[$numfiles] $enddate`
else
  set tstart = `${LAGRANTO}/gettidiff $datfiles[$numfiles] $startdate`
  set tend   = `${LAGRANTO}/gettidiff $enddate $datfiles[1]`
endif
...
#   Check availability on P file
    foreach var ( `${LAGRANTO}/getvars ${charp}$datfiles[1]` )
       if ( "${var}" == "${tvar}" ) then
          set tcompfl=0
	  set tprefix="P"
       endif
...
#   Check availability on S file 
    if ( ${sfiles} == 1 ) then
       foreach var ( `${LAGRANTO}/getvars ${chars}$datfiles[1]` )
         if ( "${var}" == "${tvar}" ) then
            set tcompfl=0
	    set tprefix="S"
         endif
...
foreach v ( $vars )
  if ( $calf[1] == 0 ) then
    ...
    ${LAGRANTO}/getvars $tfil[1]$datfiles[1]

    set flag=`${LAGRANTO}/getvars $tfil[1]$datfiles[1] | grep " $v0 " | wc -l`
...
if ( "${changet}" == "true" ) then
  ...
  echo   
  foreach i ( $datfiles )
    ${LAGRANTO}/changet.sh ${refdate} ${charp}${i}
  end
  if ( ${sfiles} == 1 ) then
    foreach i ( $datfiles )
      ${LAGRANTO}/changet.sh ${refdate} ${chars}${i}
    end
  endif
endif
...
echo $ntrace                                               >> $parfile
cat ${tvfile}                                              >> $parfile
${LAGRANTO}/getvars ${charp}$datfiles[1] | wc -l   >> $parfile
${LAGRANTO}/getvars ${charp}$datfiles[1]           >> $parfile
if ( $sfiles == 1 ) then
  ${LAGRANTO}/getvars ${chars}$datfiles[1] | wc -l >> $parfile
  ${LAGRANTO}/getvars ${chars}$datfiles[1]         >> $parfile
else
  echo 0                                                   >> $parfile
endif
