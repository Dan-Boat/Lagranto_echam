...
echo "PROGRAM CALTRA        : ${LAGRANTO}/caltra"
...
if ( "${isen}" == "yes" ) then  
   foreach date ( ${datfiles} )
      if ( -f ${chars}${date} ) then
         set ok = `${LAGRANTO}/getvars ${chars}${date} | grep TH` 
         if ( "${ok}" == "" ) set thonS = 0
      else
         set thonS = 0
      endif
...
if ( "${modlev}" == "yes" ) then  
   set flonp = 1
   foreach date ( ${datfiles} )
       set ok = `${LAGRANTO}/getvars ${charp}${date} | grep P` 
       if ( "${ok}" == "" ) set flonp = 0
       set ok = `${LAGRANTO}/getvars ${charp}${date} | grep PS` 
       if ( "${ok}" == "" ) set flonp = 0
   end
   if ( ${flonp} == 0 ) then
...
set omflag=`${LAGRANTO}/getvars $file | grep " OMEGA " | wc -l`
if ( $omflag != 1 ) then
...
set wmin  = `${LAGRANTO}/getmima $file OMEGA P850 | awk '{print $1}'`
set wmax  = `${LAGRANTO}/getmima $file OMEGA P850 | awk '{print $2}'`
...
if ( "${format}" == "0" ) then
   ...
   set ntra = `${LAGRANTO}/trainfo.sh ${startf} ntra`
   set ncol = `${LAGRANTO}/trainfo.sh ${startf} ncol`
   set ntim = `${LAGRANTO}/trainfo.sh ${startf} ntim`
